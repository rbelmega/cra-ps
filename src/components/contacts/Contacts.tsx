...
const icons: Record<string, React.ReactNode> = {
  ...
  facebook: (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path
        fill="currentColor"
        d="M15.5 8H18V5h-2.5c-2.5 0-4.3 1.8-4.3 4.2V12H9v3h2.2v6h3V15H17l.5-3h-3.3v-2.7c0-.8.4-1.3 1.3-1.3z"
      />
    </svg>
  ),
  github: (
        ...
        <li key={contact.name} className={styles.contactItem}>
          ...
          >
            <span className={styles.contactIcon} aria-hidden="true">
              {icons[contact.name.toLowerCase()] ?? <span className={styles.contactLetter}>{contact.letter}</span>}
            </span>
